...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  if (variant === 'primary') {
    return (
      ...
      <motion.button 
        ...
        whileTap={{ scale: 0.98 }}
        className={`${baseStyles} ${sizeStyles[size]} ${className} bg-emerald-950/80 border border-emerald-500/50`}
        {...props as any}
      >
        {/* Shimmer / Beam Effect */}
        <motion.div 
  ...
  return (
    <motion.button 
      whileHover={{ scale: 1.02 }}
      whileTap={{ scale: 0.98 }}
      className={`${baseStyles} ${sizeStyles[size]} ${variants['secondary']} ${className}`}
      {...props as any}
    >
       <motion.div 
