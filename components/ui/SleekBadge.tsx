...
export const SleekBadge: React.FC<SleekBadgeProps> = ({ children, color = 'emerald', className = '', noDot = false }) => {
  const styles = {
    emerald: {
    ...
    >
      {!noDot && (
        <span className="relative flex h-1.5 w-1.5">
          <span className={`relative inline-flex rounded-full h-1.5 w-1.5 ${activeStyle.dot} shadow-[0_0_4px_currentColor]`}></span>
        </span>
      )}
